-- accept_delivery_offer atualizava orders.status direto pra 'DRIVER_ASSIGNED'
-- e gravava no order_status_history uma transição fixa "SEARCHING_DRIVER ->
-- DRIVER_ASSIGNED" — mas se o entregador aceita a oferta ANTES da
-- distribuidora clicar "Pronto" (fluxo normal desde que a busca passou a
-- começar no "Aceitar"), o pedido na verdade ainda está em 'PREPARING'
-- nesse momento. O histórico ficava com uma transição fantasma que nunca
-- aconteceu, e pulava registrar READY_FOR_PICKUP/SEARCHING_DRIVER.
--
-- Corrige lendo o status atual do pedido e encadeando as transições
-- válidas de verdade (PREPARING -> READY_FOR_PICKUP -> SEARCHING_DRIVER ->
-- DRIVER_ASSIGNED) só a partir de onde o pedido realmente está — se a
-- distribuidora já tinha clicado "Pronto" antes, o comportamento é
-- idêntico ao de antes (só o passo final).
